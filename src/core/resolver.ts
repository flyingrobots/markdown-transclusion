...
import {
  ...
  substituteVariables as substituteVars,
  resolveExtensions
} from './coreUtils';
...
/**
 * Re-export substituteVariables from coreUtils for backward compatibility
 */
export { substituteVariables } from './coreUtils';
...
export function resolvePath(
  ...
  try {
      ...
      for (const relativePath of pathsToTry) {
        ...
        if (existingFile) {
          return {
            ...
            resolvedPath: existingFile.absolutePath,
            exists: true,
            originalPath: reference
          };
        }
    ...
    const absolutePath = resolveToAbsolutePath(substitutedReference, primaryBase);
    return {
