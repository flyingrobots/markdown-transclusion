...
echo "----------------------------------------"
if docker compose -f docker-compose.test.yml run --rm lint 2>&1; then
    echo "----------------------------------------"
...
echo "----------------------------------------"
if docker compose -f docker-compose.test.yml run --rm type-check 2>&1; then
    echo "----------------------------------------"
...
for version in "${NODE_VERSIONS[@]}"; do
    ...
    echo "----------------------------------------"
    if docker compose -f docker-compose.test.yml run --rm "test-node${version}" 2>&1; then
        echo "----------------------------------------"
